	...
	public ConvertedInvoice addDiscountToInvoice(ConvertedInvoice cinvoice, Context context, Integer counter, String discountName) {
		...
		if (!discountType.equalsIgnoreCase("bonus")) {
			while (ci.hasNext()) {
				...
				if (groupname.contains(context.priceList.getGroup(product.getProductName()))) {
					productInGroupListQuantity++;
					if ((productInGroupListQuantity % quantity == 0) && ((productInGroupListQuantity / quantity) <= counter)) {
						if (discountAmount >= 0) {
